...
if(idAmigo != null){
	ResultSet rs = DataBaseUtils.executeQuery("SELECT p.idpostagem AS idpostagem, p.idpostagem_comentario AS postagemComentario, p.postagem AS postagem, p.data AS data, u.idusuario AS idUsuarioPostagem, CONCAT(u.nome,' ', u.sobrenome ) AS usuarioPostagem, u.foto AS fotoUsuario,getCurtidas(p.idpostagem) AS curtidas, getCurtidores(p.idpostagem) AS curtidores,IF((SELECT c.idcurtida FROM curtida c WHERE c.idpostagem = p.idpostagem AND c.idusuario = "+idUser+") IS NULL, 0, 1)  AS curti FROM postagem p LEFT JOIN usuario u ON u.idusuario = p.idusuario WHERE p.idpostagem_comentario IS NULL AND p.idusuario = "+idAmigo+" ORDER BY p.idpostagem DESC LIMIT 50;");
		...
		out.println("<div class='card col l12 s12'>");
			...
			out.println("<div class='col l10 s10 right'>");
				...
				out.println("<div class='col s8 l8 left'>");
					out.println("<span id=\"curtidas-"+rs.getInt("idpostagem")+"\">"+rs.getInt("curtidas")+"</span> pessoas curtiram isso");
				out.println("</div>");
					...
					out.println("<p id=\"curtidores-"+rs.getInt("idpostagem")+"\" class='truncate curtidores' onmouseover='$(this).toggleClass(\"truncate\")' onmouseout='$(this).toggleClass(\"truncate\")'>Curtidores: "+curtidores+"</p>");
					out.println("</div>");
			...
			while(rs2.next()){
				...
				out.println("<div class='card col l12 s12 comentario right'>");
					...
					out.println("<div class='col s8 l8 left'>");
						out.println("<span id=\"curtidas-"+rs2.getInt("idpostagem")+"\">"+rs2.getInt("curtidas")+"</span> pessoas curtiram isso");
						
						String curtidores2 = "";
						if(rs2.getString("curtidores") != null){
							curtidores2 = rs2.getString("curtidores");
						}
						out.println("<p id=\"curtidores-"+rs2.getInt("idpostagem")+"\" class='truncate curtidores' onmouseover='$(this).toggleClass(\"truncate\")' onmouseout='$(this).toggleClass(\"truncate\")'>Curtidores: "+curtidores2+"</p>");
					out.println("</div>");
					out.println("</div>");
				out.println("</div>");
				out.println("</div>");
	...
	ResultSet rs = DataBaseUtils.executeQuery("SELECT p.idpostagem AS idpostagem, p.idpostagem_comentario AS postagemComentario, p.postagem AS postagem, p.data AS data, u.idusuario AS idUsuarioPostagem, CONCAT(u.nome,' ', u.sobrenome ) AS usuarioPostagem, u.foto AS fotoUsuario,getCurtidas(p.idpostagem) AS curtidas, getCurtidores(p.idpostagem) AS curtidores,IF((SELECT c.idcurtida FROM curtida c WHERE c.idpostagem = p.idpostagem AND c.idusuario = "+idUser+") IS NULL, 0, 1)  AS curti FROM postagem p LEFT JOIN usuario u ON u.idusuario = p.idusuario WHERE p.idpostagem_comentario IS NULL ORDER BY p.idpostagem DESC LIMIT 50;");

	while(rs.next()){
		if(rs.isFirst()){
			session.setAttribute("primeiroPost", rs.getString("idpostagem"));
		}
		out.println("<div id='divPost-"+rs.getInt("idpostagem")+"'>");
		out.println("<div class='card col l12 s12'>");
			out.println("<img src='"+rs.getString("fotoUsuario")+"' style='height: 100px; width: 100px;' class='circle col l2 s2 image-centralizar'>");
			...
			out.println("<div class='col l10 s10 right'>");
				...
				out.println("<div id='io"+rs.getInt("idpostagem")+"'>");
				out.println("<div class='col s8 l8 left'>");
					out.println("<span id=\"curtidas-"+rs.getInt("idpostagem")+"\">"+rs.getInt("curtidas")+"</span> pessoas curtiram isso");
				out.println("</div>");
